...
class BeersFollowedByBreweriesParser: ParserProtocol {
    ...
    let breweryDesigner: BreweryDesignerProtocol
    let beerDesigner: BeerDesignerProtocol

    init(withBeerDesigner: BeerDesignerProtocol,
         with breweryDesigner: BreweryDesignerProtocol) {
        self.beerDesigner = withBeerDesigner
    ...
    func parse(response : NSDictionary,
        ...
        DispatchQueue.global(qos: .userInteractive).async {
            // FIXME
            print("Beginning to process beers followed by breweries")
            let begin = Date()
            createBeerLoop: for beer in beerArray {
                ...
                breweryLoop: for brewery in breweriesArray {
                    ...
                    self.breweryDesigner.createBreweryObject(breweryDict: breweryDict,
                                                                         locationDict: locDic,
                                                                         brewersID: brewersID,
                                                                         style: styleID) {
                                                                            (Brewery) -> Void in
                    ...
                    self.beerDesigner.createBeerObject(beer: beer, brewerID: brewersID) {
                        (Beer) -> Void in
                    }
            ...
            // FIXME
            print("Finished beer followed by breweries loop. \(Date().timeIntervalSince(begin)/60)")
            
            NSLog("NSLOG SwiftyBeaver")
        }
    }
}
