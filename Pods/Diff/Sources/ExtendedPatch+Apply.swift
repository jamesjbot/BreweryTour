    ...
    public func apply(_ patch: [ExtendedPatch<Iterator.Element>]) -> Self {
        ...
        for change in patch {
            ...
            case let .insertion(i, element):
                let target = mutableSelf.index(mutableSelf.startIndex, offsetBy: IndexDistance(UInt64(i)))
                mutableSelf.insert(element, at: target)
            case let .deletion(i):
                let target = mutableSelf.index(mutableSelf.startIndex, offsetBy: IndexDistance(UInt64(i)))
                mutableSelf.remove(at: target)
            case let .move(from, to):
                let fromIndex = mutableSelf.index(mutableSelf.startIndex, offsetBy: IndexDistance(UInt64(from)))
                let toIndex = mutableSelf.index(mutableSelf.startIndex, offsetBy: IndexDistance(UInt64(to)))
...
public extension String {
    ...
    public func apply(_ patch: [ExtendedPatch<String.CharacterView.Iterator.Element>]) -> String {
        ...
        for change in patch {
            ...
            case let .move(from, to):
                let fromIndex = mutableSelf.index(mutableSelf.startIndex, offsetBy: IndexDistance(UInt64(from)))
                let toIndex = mutableSelf.index(mutableSelf.startIndex, offsetBy: IndexDistance(Int64(to)))
                let element = mutableSelf.remove(at: fromIndex)
                mutableSelf.insert(element, at: toIndex)
            }
        }
